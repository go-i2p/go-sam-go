...
GO_SAM_GO_TAG_HASH=$(collecthash go-sam-go) # 10
echo "go-sam-go tag hash: $GO_SAM_GO_TAG_HASH" 1>&2
ONRAMP_TAG_HASH=$(collecthash onramp) # 11
echo "onramp tag hash: $ONRAMP_TAG_HASH" 1>&2
GO_I2PTUNNEL_TAG_HASH=$(collecthash go-i2ptunnel) # 12
echo "go-i2ptunnel tag hash: $GO_I2PTUNNEL_TAG_HASH" 1>&2
...
update_our_packages() {
  ...
  echo go get "github.com/go-i2p/go-sam-go@$GO_SAM_GO_TAG_HASH" 1>&2
  go get "github.com/go-i2p/go-sam-go@$GO_SAM_GO_TAG_HASH" >/dev/null 2>/dev/null || true
  echo go get "github.com/go-i2p/onramp@$ONRAMP_TAG_HASH" 1>&2
  go get "github.com/go-i2p/onramp@$ONRAMP_TAG_HASH" >/dev/null 2>/dev/null || true
  echo go get "github.com/go-i2p/go-i2ptunnel@$GO_I2PTUNNEL_TAG_HASH" 1>&2
  go get "github.com/go-i2p/go-i2ptunnel@$GO_I2PTUNNEL_TAG_HASH" >/dev/null 2>/dev/null || true
  go mod tidy -v 1>&2
  go build -v ./... 1>&2
...
GO_SAM_GO_TAG_HASH=$(tagandrelease go-sam-go) # 12
ONRAMP_TAG_HASH=$(tagandrelease onramp) # 13
GO_I2PTUNNEL_TAG_HASH=$(tagandrelease go-i2ptunnel) # 14
